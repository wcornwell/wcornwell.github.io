...
- **ORCID is the authoritative source** for which works exist (176, of which 165 have DOIs). Crossref is used *only* to fill in author lists, which ORCID's work summaries omit. Do **not** switch to Crossref's `filter=orcid:` search as the primary source — it only returns works where a publisher deposited the ORCID iD (68 of 176). Crossref is queried in batches of 20 DOIs via OR'd `filter=doi:` params, so all 165 take ~9 requests.
...
- **11 works have no DOI**, so they get no author list (authors come from Crossref, which is keyed on DOI). They still render, with the title linked to ORCID's `url` when one exists. **Fix these upstream in ORCID, not with a local patch file** — a patch would be a second source of truth that drifts, whereas an ORCID edit is picked up automatically by the next weekly refresh. As of 2026-07-20 six have known DOIs worth adding to the ORCID record: Bryologist 2020 `10.1639/0007-2745-123.3.443`, Functional Ecology 2015 `10.1111/1365-2435.12449`, Plant and Soil 2015 `10.1007/s11104-015-2595-1`, Ecology 2006 `10.1890/0012-9658(2006)87[1465:attfhf]2.0.co;2`, Ecology 2002 `10.2307/3071768`, Am J Botany 2001 `10.2307/3558359`. The rest legitimately have none (two AGU conference abstracts, a book chapter, a PhD thesis); the 2015 *Model adequacy* paper needs its published Am Nat DOI looked up by hand, since a Crossref title search returns the bioRxiv preprint instead.
  - If you ever re-run that title search: **verify each hit's year and journal before trusting it.** Of 11 title lookups, 4 returned confidently-wrong papers. Never bulk-import Crossref title matches into the ORCID record.
